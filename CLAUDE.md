...
* Проверка и публикация (шаг 5b, миграция 011) — отправка на проверку, 3 одобрения, «запросить изменения», публикация, публичные «Создано / Одобрено». См. раздел «Проверка лабораторных». Ревизии структуры опубликованных (5c) — ещё не сделаны.
...
* Доступ (RLS):
  ...
  * `lab_reviews` (одобрения и запросы изменений): прямого доступа нет; админы читают через `lab_review_history`, публично — только имена/места работы/дата одобривших через `lab_credits`.
...
└── components/
    ...
    │                   LabCredits (публичные «Создано / Одобрено»)
...
* **Опубликованные:** только косметика — тексты (название, описание, оборудование), подписи, подсказки, порядок полей и вариантов, значок, регион, сложность, центр/зум, пресет цветов, статус. Структура (новые/удалённые поля и варианты, архив, обязательность, min/max/decimals, длина текста, основное поле) и **протокол** → `structural_change` (триггеры, в т. ч. из SQL Editor); в 5c станут ревизией с 3 одобрениями. Адрес (`slug`) после публикации закреплён. Подписи опубликованных не могут стать пустыми.
