...
* Роли (шаг 4b) — владелец / главные админы / админы, назначение и каскадное снятие, переименование, журнал; всё проверяется в базе (миграция 009). Панель «Администрирование» — в своём профиле у админов. См. раздел «Роли».
...
* Доступ (RLS):
  ...
  * `campaigns`, `campaign_fields`, `campaign_field_options`: все читают. Добавлять/изменять — любой админ (`is_admin()`; шаг 5 уточнит). Удалять кампанию — главный админ и владелец любую, обычный админ — только свою (`created_by`). Поля/варианты — любой админ (правила 004 действуют). У `campaigns` чтение по списку колонок (`created_by` не публичен): **новой колонке нужен свой `grant select (колонка)`**.
...
└── components/
    ...
    ├── admin/        — AdminPanel (вкладки Пользователи·Журнал), UserList, RevokeConfirm (список теряющих права), RenameForm, RoleLog
...
* Коды ошибок: `not_logged_in`, `not_allowed`, `bad_target`, `no_username`, `chain_changed`, `unchanged`, `username_taken`, `invalid_username` (detail = код правила имени) → `admin.errors.*` / `auth.errors.*` в `strings.js`.
...
* Обещания в тексте, которые держит код: журнал ролей и переименований видят только админы, при удалении аккаунта имена из него стираются; `campaigns.created_by` не публичен; журнал входа Supabase (`auth.audit_log_entries`, там IP) — до 30 дней, хешированные IP в `private.rate_limit_hits` — сутки (обе чистки — задача pg_cron из 008); писем, кроме сброса пароля и подтверждения email, нет; в экспорте нет имён пользователей; координаты — 5 знаков (~1 м).
