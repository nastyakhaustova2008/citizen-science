...
* Редактор лабораторных (шаг 5a, миграция 010) — профиль админа, черновики, редактор (описание, поля, протокол, предпросмотр), косметическая правка опубликованных, журнал лабораторных. См. раздел «Редактор лабораторных». Проверка и публикация (5b) и ревизии структуры опубликованных (5c) — ещё не сделаны.
...
* Доступ (RLS):
  ...
  * `lab_events` (журнал лабораторных): читают админы через `lab_log`, пишут только функции 010.
...
* **Статусы:** `campaigns.publication` = `draft` | `in_review` | `published` (старые — `published`) — отдельно от `status` (`collecting` | `completed`, имеет смысл только у опубликованных, меняется в редакторе). `in_review` пока не используется (5b).
...
* **Журнал** `lab_events`: `create`, `edit`, `edit_published`, `delete`; `details` = какие колонки/поля изменились (`lab_snapshot` + `lab_diff`). Название копируется, чтобы запись читалась после удаления лабораторной; имён пользователей нет (только `actor_id`, при удалении аккаунта — null).
...
* Обещания в тексте, которые держит код: журнал ролей и переименований видят только админы, при удалении аккаунта имена из него стираются; профиль админа видят админы, публично — только на лабораторных, которые он создал/одобрил (и после снятия роли), удаляется с аккаунтом; журнал лабораторных видят только админы, имён в нём нет; `campaigns.created_by` не публичен; журнал входа Supabase (`auth.audit_log_entries`, там IP) — до 30 дней, хешированные IP в `private.rate_limit_hits` — сутки (обе чистки — задача pg_cron из 008); писем, кроме сброса пароля и подтверждения email, нет; в экспорте нет имён пользователей; координаты — 5 знаков (~1 м).
