...
import { photoLog } from '../../lib/photosApi';
import { EmptyState, ErrorBlock, SkeletonText } from '../primitives';
import { isolate } from '../auth/AuthUI';

const PAGE = 30;

/** Photo moderation (who approved / hid / removed whose photo, on which lab, when). Admins only. No images. */
export default function PhotoLog() {
  ...
  const load = useCallback(async () => {
    ...
    setError(false);
    try {
      const rows = await photoLog({ limit: PAGE });
      setEvents(rows);
  ...
  async function loadMore() {
    setMoreBusy(true);
    try {
      const rows = await photoLog({ limit: PAGE, before: events[events.length - 1].id });
  ...
  if (events.length === 0) return <EmptyState title={t('photos.log.empty')} />;
  ...
  return (
    <li className="py-2.5 text-sm">
      <p className="flex flex-wrap items-baseline gap-x-1.5">
        {!e.actorId && (e.action === 'auto_hide' || e.action === 'expire') ? (
        ...
        <span className="text-ink-soft dark:text-paper/80">
          {t(`photos.log.actions.${e.action}`, { count: e.reports, author: isolate(author) })}
        </span>
        {e.slug ? (
          <Link
      ...
      <p className="mt-0.5 text-xs text-ink-faint">
        {e.reason && e.reason !== 'expired' && <span className="me-2">{t(`photos.removeReasons.${e.reason}`)}</span>}
